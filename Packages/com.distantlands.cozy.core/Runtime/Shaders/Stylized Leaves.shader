...
Shader "Distant Lands/Cozy/HDRP/Demo/Stlylized Leaves"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float3 hsvTorgb101 = RGBToHSV( inputMesh.ase_color.rgb );
				float3 ase_worldPos = GetAbsolutePositionWS( TransformObjectToWorld( (inputMesh.positionOS).xyz ) );
				float2 texCoord29 = inputMesh.ase_texcoord.xy * float2( 1,1 ) + ( ase_worldPos + ( _FlutterSpeed * CZY_WindTime ) ).xy;
				float simplePerlin2D27 = snoise( texCoord29*_FlutterScale );
				float4 transform108 = mul(GetObjectToWorldMatrix(),float4( 0,0,0,1 ));
				transform108.xyz = GetAbsolutePositionWS((transform108).xyz);
				float2 appendResult113 = (float2(( ( transform108.x + transform108.z ) / 10.0 ) , CZY_WindTime));
				float2 texCoord114 = inputMesh.ase_texcoord.xy * float2( 0,0 ) + appendResult113;
				float simplePerlin2D122 = snoise( texCoord114*_WindNoiseSize );
				float3 temp_output_117_0 = ( CZY_WindDirection * simplePerlin2D122 * ( 0.1 * inputMesh.positionOS.y ) );
						...
						)
			{
				...
				float clampResult13 = clamp( ( ( packedInput.ase_texcoord6.xyz.y - _GradientOffset ) * _GradientSmoothness ) , 0.0 , 1.0 );
				...
				transform5_g21.xyz = GetAbsolutePositionWS((transform5_g21).xyz);
				...
				float4 appendResult190 = (float4(saturate( hsvTorgb4_g21 ) , 1.0));
				float3 ase_worldPos = GetAbsolutePositionWS( positionRWS );
				float2 appendResult3_g22 = (float2(ase_worldPos.x , ase_worldPos.z));
				...
				float4 lerpResult19_g22 = lerp( ( CZY_SnowColor * tex2D( CZY_SnowTexture, uvCZY_SnowTexture ) ) , ( ( tex2DNode84 * lerpResult7 ) * appendResult190 ) , ( ( pow( ( pow( normalWS.y , 7.0 ) * ( simplePerlin2D7_g22 * ( 1.0 - voroi5_g22 ) ) ) , 0.5 ) * _SnowAttraction ) > ( 1.0 - CZY_SnowAmount ) ? 0.0 : 1.0 ));
				...
				float3 hsvTorgb3_g23 = RGBToHSV( _BottomColor.rgb );
				float4 transform5_g23 = mul(GetObjectToWorldMatrix(),float4( 0,0,0,1 ));
				transform5_g23.xyz = GetAbsolutePositionWS((transform5_g23).xyz);
				float simplePerlin2D7_g23 = snoise( transform5_g23.xy*( 1.0 / _VariationScale ) );
				float3 hsvTorgb4_g23 = HSVToRGB( float3(( hsvTorgb3_g23.x + ( simplePerlin2D7_g23 * _VariationAmount ) ),hsvTorgb3_g23.y,hsvTorgb3_g23.z) );
				float4 appendResult191 = (float4(saturate( hsvTorgb4_g23 ) , 1.0));
				float3 hsvTorgb3_g24 = RGBToHSV( _TopColor.rgb );
				float4 transform5_g24 = mul(GetObjectToWorldMatrix(),float4( 0,0,0,1 ));
				transform5_g24.xyz = GetAbsolutePositionWS((transform5_g24).xyz);
				float simplePerlin2D7_g24 = snoise( transform5_g24.xy*( 1.0 / _VariationScale ) );
				float3 hsvTorgb4_g24 = HSVToRGB( float3(( hsvTorgb3_g24.x + ( simplePerlin2D7_g24 * _VariationAmount ) ),hsvTorgb3_g24.y,hsvTorgb3_g24.z) );
				float4 appendResult192 = (float4(saturate( hsvTorgb4_g24 ) , 1.0));
				float4 lerpResult72 = lerp( ( ( _LowerLightTransmission * ase_lightColor ) * appendResult191 ) , ( ( _UpperLightTransmission * ase_lightColor ) * appendResult192 ) , SunDirection97);
				
				float4 transform2_g17 = mul(GetObjectToWorldMatrix(),float4( 0,0,0,1 ));
				transform2_g17.xyz = GetAbsolutePositionWS((transform2_g17).xyz);
				float simplePerlin2D3_g17 = snoise( transform2_g17.xy*( 1.0 / _VariationScale ) );
				simplePerlin2D3_g17 = simplePerlin2D3_g17*0.5 + 0.5;
				
				surfaceDescription.BaseColor = lerpResult19_g22.rgb;
				...
				surfaceDescription.Emission = ( ( tex2DNode84 * lerpResult72 ) * appendResult190 ).rgb;
				surfaceDescription.Smoothness = 0.0;
				surfaceDescription.Occlusion = 1;
				surfaceDescription.Alpha = ( tex2DNode84.a - ( simplePerlin2D3_g17 * _AlphaVariation ) );
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh  )
			{
				...
				float3 hsvTorgb101 = RGBToHSV( inputMesh.ase_color.rgb );
				float3 ase_worldPos = GetAbsolutePositionWS( TransformObjectToWorld( (inputMesh.positionOS).xyz ) );
				float2 texCoord29 = inputMesh.uv0.xy * float2( 1,1 ) + ( ase_worldPos + ( _FlutterSpeed * CZY_WindTime ) ).xy;
				float simplePerlin2D27 = snoise( texCoord29*_FlutterScale );
				float4 transform108 = mul(GetObjectToWorldMatrix(),float4( 0,0,0,1 ));
				transform108.xyz = GetAbsolutePositionWS((transform108).xyz);
				float2 appendResult113 = (float2(( ( transform108.x + transform108.z ) / 10.0 ) , CZY_WindTime));
				float2 texCoord114 = inputMesh.uv0.xy * float2( 0,0 ) + appendResult113;
				float simplePerlin2D122 = snoise( texCoord114*_WindNoiseSize );
				float3 temp_output_117_0 = ( CZY_WindDirection * simplePerlin2D122 * ( 0.1 * inputMesh.positionOS.y ) );
				float3 MainWind124 = ( _WindStrength * temp_output_117_0 );
				float3 FinalWind126 = ( ( hsvTorgb101.z * simplePerlin2D27 * _FlutterAmount * temp_output_117_0 ) + MainWind124 );
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float4 tex2DNode84 = tex2D( _Texture, uv_Texture );
				float clampResult13 = clamp( ( ( packedInput.ase_texcoord3.xyz.y - _GradientOffset ) * _GradientSmoothness ) , 0.0 , 1.0 );
				float SunDirection97 = clampResult13;
				float4 lerpResult7 = lerp( _BottomColor , _TopColor , SunDirection97);
				float3 hsvTorgb3_g21 = RGBToHSV( _FilterColor.rgb );
				float4 transform5_g21 = mul(GetObjectToWorldMatrix(),float4( 0,0,0,1 ));
				...
				float2 uv5_g22 = 0;
				float voroi5_g22 = voronoi5_g22( coords5_g22, time5_g22, id5_g22, uv5_g22, 0, voronoiSmoothId5_g22 );
				float4 lerpResult19_g22 = lerp( ( CZY_SnowColor * tex2D( CZY_SnowTexture, uvCZY_SnowTexture ) ) , ( ( tex2DNode84 * lerpResult7 ) * appendResult190 ) , ( ( pow( ( pow( ase_worldNormal.y , 7.0 ) * ( simplePerlin2D7_g22 * ( 1.0 - voroi5_g22 ) ) ) , 0.5 ) * _SnowAttraction ) > ( 1.0 - CZY_SnowAmount ) ? 0.0 : 1.0 ));
		...
		Pass
		{
						...
						)
			{
				...
				float4 tex2DNode84 = tex2D( _Texture, uv_Texture );
				float clampResult13 = clamp( ( ( packedInput.ase_texcoord8.xyz.y - _GradientOffset ) * _GradientSmoothness ) , 0.0 , 1.0 );
				float SunDirection97 = clampResult13;
				float4 lerpResult7 = lerp( _BottomColor , _TopColor , SunDirection97);
				float3 hsvTorgb3_g21 = RGBToHSV( _FilterColor.rgb );
				float4 transform5_g21 = mul(GetObjectToWorldMatrix(),float4( 0,0,0,1 ));
...
Node;AmplifyShaderEditor.RangedFloatNode;110;-3994.671,1984.126;Inherit;False;Global;CZY_WindTime;CZY_WindTime;9;0;Create;True;0;0;0;False;0;False;1;79.71432;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.Vector3Node;119;-2796.179,1667.281;Inherit;False;Global;CZY_WindDirection;CZY_WindDirection;11;0;Create;True;0;0;0;False;0;False;0,0,0;0.4176402,0,0.2749121;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
...
Node;AmplifyShaderEditor.CommentaryNode;74;-4143.363,-1496.772;Inherit;False;1287.013;1235.088;Lerp between two colors for the color of the leaves;18;7;72;98;21;16;23;19;151;18;17;6;20;22;152;1;160;191;192;;1,1,1,1;0;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;147;-1507.326,-681.9669;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;FLOAT4;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;146;-1508.832,-787.4256;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;FLOAT4;0,0,0,0;False;1;COLOR;0
...
WireConnection;21;1;191;0
WireConnection;191;0;160;0
WireConnection;192;0;161;0
WireConnection;161;12;151;0
WireConnection;161;13;152;0
WireConnection;161;1;6;0
WireConnection;16;0;19;0
WireConnection;16;1;192;0
WireConnection;178;0;166;0
WireConnection;178;6;146;0
WireConnection;178;7;176;0
WireConnection;178;9;165;0
WireConnection;178;10;86;0
WireConnection;178;11;141;0
ASEEND*/
//CHKSM=0BD7578B8F40BD2709C5396C08CD510E4D9D08DD